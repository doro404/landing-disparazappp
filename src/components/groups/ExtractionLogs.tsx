...
const LEVEL_COLOR: Record<GroupLogLine['level'], string> = {
  info: 'text-sky-400',
...
const LEVEL_LABEL: Record<GroupLogLine['level'], string> = {
  info: '[INFO   ]',
  success: '[SUCESSO]',
  warn: '[AVISO  ]',
  error: '[ERRO   ]',
...
export function ExtractionLogs({ logs, onClear }: Props) {
    ...
    <div className="rounded-xl border border-neutral-800 bg-neutral-950 flex flex-col overflow-hidden" style={{ height: '9rem' }}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-neutral-800 bg-neutral-900/80 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono text-wa-green/80">group-extractor</span>
        ...
        <div className="space-y-0.5 font-mono text-xs">
          {logs.length === 0 ? (
            <p className="text-neutral-600 italic">
              <span className="text-wa-green/40">$</span> aguardando início da extração...
            </p>
          ) : (
